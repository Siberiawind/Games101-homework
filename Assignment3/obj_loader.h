...
// Namespace: OBJ1
//
// Description: The namespace that holds everything that is
// needed and used for the OBJ Model Loader
...
namespace obj1
{
		...
		bool LoadFile(std::string Path)
		{
			...
			while (std::getline(file, curline))
			{
				...
				if (algorithm::firstToken(curline) == "usemtl")
				{
					...
					// Create new Mesh, if Material chagnes within a group
					if (!Indices.empty() && !Vertices.empty())
					{
						// Create Mesh
						tempMesh = Mesh(Vertices, Indices);
	...
	private:
		...
		// Triangluate a list of vertices into a face by printing
